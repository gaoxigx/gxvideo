 ...
 * @package Admin\Logic
 */
class MovieCatLogic extends RelationModel
{
     ...
     * @return  mix
     */
    public function movie_cat_list($table,$id = 0, $selected = 0, $re_type = true, $level = 0)
    {
        static $res = NULL;
        ...
        if ($res === NULL)
        {
            ...
            if ($data === false)
            {
                $sql = "SELECT c.*, COUNT(s.id) AS has_children FROM __PREFIX__".$table." AS c".
                    " LEFT JOIN __PREFIX__".$table." AS s ON s.parent_id=c.id".
                    " GROUP BY c.id  ORDER BY parent_id, sort_order";
                $res = D($table)->query($sql);
        ...
        $options = $this->movie_cat_options($id, $res); // 获得指定分类下的子分类的数组
        ...
        else
        {
            foreach ($options AS $key => $value)
            {
                ///$options[$key]['url'] = build_uri('movie_cat', array('acid' => $value['id']), $value['cat_name']);
            }
            return $options;
        }
    }
     ...
     * @return  void
     */
    public function movie_cat_options($spec_id, $arr)
    {
